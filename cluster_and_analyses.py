...
def helper_numeric_p_vals(df_holder, func, dfs, col, p_val_numeric):
    """
    ...
    for col in numric_cols:
        ...
        if not isinstance(dfs[0][col], pd.Series):
            ...
            for i in range(len(dfs[0][col].columns)):
                ...
                helper_numeric_p_vals(df_holder, func, dfs, col_name, p_val_numeric)
        else:
            for i in range(len(dfs)):
                    df_holder[i] = dfs[i][col].dropna()
            helper_numeric_p_vals(df_holder,func, dfs,col,p_val_numeric)
...
def calc_c_index(df_origin, df_all_num, df_all_cat, clusters, times_name, outcome_name):
    """
    ...
    df_all = main.pd.concat([df_all_num, df_all_cat], axis=1)
    ...
    for j in range(len(events)):
        if main.pd.isna(raw_times[j]) or (not raw_times[j]) or (raw_times[j] < 0):
            events[j] = 0

    binary_table = main.oneHot(clusters)
    ...
    cph.print_summary()
    c_index = main.concordance_index(df_all['T'], -cph.predict_partial_hazard(df_all), df_all['E'])
    return c_index
...
def logrank(df_origin, clusters, times_name, outcome_name):
    ...
    for j in range(len(events)):
        if main.pd.isna(raw_times[j]) or (not raw_times[j]) or (raw_times[j] < 0):
            events[j] = 0
    result = main.multivariate_logrank_test(times, clusters, events)
    return result.p_value


def make_consensus(n_clusters, df_all_num, df_all_cat,gamma=None, r=1):
    """
    ...
    """
    consensus = consensus_with_kprototype.ConsensusCluster(cluster=main.KMeans, L=n_clusters, K=n_clusters + 1, H=50, resample_proportion=r)
    consensus.fit(df_all_num, df_all_num, df_all_cat, gamma, verbose=False)

    lables = consensus.predict()
    for i in range(n_clusters):
        print(f"len of cluster {i} = {len([j for j, e in enumerate(list(map(int, lables))) if e == i])}")

    return lables


def analyses(labels, df_all, df_all_num, df_all_cat, path, outcomes):
    """
    ...
    get_pvals(df_all_num.columns, df_all_cat.columns, outcomes, file_name=f'{path}{name} - {n_clusters}', labels=labels,
              df_all=df_all, path=path)

    # c-index for the clustering solution
    c_index = calc_c_index(df_all,df_all_num, df_all_cat, labels, "days in hospital ", "In hospital mortality")
...
def shuffle_vals(df_num, n, df_cat, start_ind= None):
    ...
    if start_ind:
        ...
        for i in range(n):
            seri = main.pd.Series(df_num[df_num.columns[start_ind+i]], index=df_num.index).sample(n=len(df_num))
        ...
        for col in cat_cols:
            seri = main.pd.Series(df_cat[col], index=df_cat.index).sample(n=len(df_cat))
    ...
    # suffle non-echo variables
        ...
        for i in indxs:
            seri = main.pd.Series(df_num[df_num.columns[i]], index=df_num.index).sample(n=len(df_num))
            seri.index = df_num.index
            df_num[df_num.columns[i]] = seri
        ran = range(len(df_cat.columns))
        indxs = random.choices(ran, k=2)
        for i in indxs:
            seri = main.pd.Series(df_cat[df_cat.columns[i]], index=df_cat.index).sample(n=len(df_cat))
            seri.index = df_cat.index
            df_cat[df_cat.columns[i]] = seri



def remove_at_random(df_numeric, num_of_vars, df_all, df_all_cat, test_echo=False, echo_start_ind = None):
    """
    ...
    for i in range(number_of_iter):
        ...
        labels = make_consensus(n_clusters, df_num_c, df_cat_c, gamma, r)

        c_indexes.append(calc_c_index(df_all, df_numeric, df_all_cat, labels, "days in hospital ", "In hospital mortality"))
    ...
    print(f"mean c index = {ave_c}; std = {std_c}")

    np_arr = main.np.array(logranks)
    ave_lr = main.np.mean(np_arr)
    std_lr = main.np.std(np_arr)
    print(f"mean log rank = {ave_lr}; std = {std_lr}")
...
if __name__ == '__main__':
    ...
    outcomes = sys.argv[4]  # list of outcomes to test

    df_full = pd.read_csv(path_to_otiginal_data)
    df_con = pd.read_csv(path_to_df_continuous)
    df_cat = pd.read_csv(path_to_df_categorical)

    n_clusters = 4
    gamma = 3  # weight parameter for k-prototype
    r = 0.85 # resampling rate for consensus clustering
    path_to_save_results = ""
    name = "test test test for consensus clustering for echo-covid"

    labels = make_consensus(n_clusters, df_con, df_cat, gamma, r)
    c_index = analyses(labels, df_full, df_con, df_cat, path_to_save_results, outcomes)
